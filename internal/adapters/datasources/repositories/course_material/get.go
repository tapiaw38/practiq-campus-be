...
const selectMaterialColumns = `
	id, course_id, section_id, uploader_id, title, description, kind, url, created_at
`
...
func (r *repository) Get(ctx context.Context, id string) (*domain.CourseMaterial, error) {
	...
	err := row.Scan(&m.ID, &m.CourseID, &m.SectionID, &m.UploaderID, &m.Title, &m.Description, &m.Kind, &m.URL, &m.CreatedAt)
